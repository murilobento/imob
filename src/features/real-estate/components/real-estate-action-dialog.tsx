...
export function RealEstateActionDialog({
  currentRow,
  open,
  onOpenChange,
  readOnly,
  ...
  useEffect(() => {
    if (open) {
      getCustomers()
        .then(setCustomers)
        .catch(() => toast.error('Falha ao carregar proprietários'))
      ...
      if (currentRow) {
        form.reset({
          ...
          rental_value: currentRow.rental_value
            ? formatCurrency(currentRow.rental_value)
            : '',
          sale_value: currentRow.sale_value
            ? formatCurrency(currentRow.sale_value)
            : '',
          condominium_value: currentRow.condominium_value
            ? formatCurrency(currentRow.condominium_value)
            : '',
          iptu_value: currentRow.iptu_value
            ? formatCurrency(currentRow.iptu_value)
            : '',
        })
      } else {
        form.reset({
  ...
  const onSubmit = async (values: RealEstateForm) => {
    ...
    try {
      const dbValues = {
        ...
        garage_spots: values.garage_spots
          ? parseInt(values.garage_spots)
          : null,
        rental_value: values.rental_value
          ? parseFloat(values.rental_value.replace(/\D/g, '')) / 100
          : null,
        sale_value: values.sale_value
          ? parseFloat(values.sale_value.replace(/\D/g, '')) / 100
          : null,
        condominium_value: values.condominium_value
          ? parseFloat(values.condominium_value.replace(/\D/g, '')) / 100
          : null,
        iptu_value: values.iptu_value
          ? parseFloat(values.iptu_value.replace(/\D/g, '')) / 100
          : null,
        ...
        <DialogHeader>
          <div className='flex w-full items-center justify-between'>
            <div className='flex items-center gap-4'>
              <DialogTitle>
                {currentRow ? 'Editar Imóvel' : 'Novo Imóvel'}
              </DialogTitle>
              ...
              <Tabs defaultValue='main' className='w-full'>
                ...
                <TabsContent value='main' className='space-y-6'>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className='md:col-span-3'>
                          ...
                          <FormControl>
                            <div className='border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-9 w-full items-center rounded-md border px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50'>
                              <Switch
                                checked={field.value}
                  ...
                  <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
                    {(form.watch('finality') === 'SALE' ||
                      form.watch('finality') === 'BOTH') && (
                      <FormField
                        control={form.control}
                        name='sale_value'
                        render={({ field }) => (
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea
                              placeholder='Cole URLs de imagens separadas por vírgula ou JSON'
                              {...field}
                              value={field.value || ''}
                              readOnly={readOnly}
                            />
